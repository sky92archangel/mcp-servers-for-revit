...
export function registerCreateTextNoteTool(server: McpServer) {
  server.tool(
    "create_text_note",
    "Create text note annotations in the current Revit view. Supports multiple text notes with custom text content, location, rotation, width, alignment, and text note type. All coordinates are in millimeters (mm). / 在当前Revit视图中创建文字注释。支持多个文字注释，可自定义文本内容、位置、旋转角度、宽度、对齐方式和文字类型。所有坐标单位为毫米（mm）。",
    {
      data: z
        .array(
          z.object({
