...
export function registerPlaceViewOnSheetTool(server) {
    server.tool("place_view_on_sheet", "Place one or more views onto sheets in Revit as viewports. Supports position control in mm, viewport type, title display, scale override, and rotation. / 在Revit中将一个或多个视图作为视口放置到图纸上。支持毫米单位的定位、视口类型、标题显示、比例覆盖和旋转。", {
        data: z
            .array(z.object({
            sheetId: z
                .number()
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Place view on sheet failed: ${error instanceof Error ? error.message : String(error)}`,
                    },
                ],
            };
        }
