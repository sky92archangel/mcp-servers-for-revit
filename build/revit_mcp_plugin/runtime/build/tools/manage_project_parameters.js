...
export function registerManageProjectParametersTool(server) {
    server.tool("manage_project_parameters", "List or add shared parameters to a Revit project. Supports listing existing project parameters and binding shared parameters. / 列出或向Revit项目添加共享参数。支持列出已有项目参数和绑定共享参数。", {
        ...
        if (args.sharedParamFile !== undefined)
            params.sharedParamFile = args.sharedParamFile;
        if (args.paramGroup !== undefined)
            params.paramGroup = args.paramGroup;
