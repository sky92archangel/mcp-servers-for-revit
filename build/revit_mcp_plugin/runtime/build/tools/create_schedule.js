...
export function registerCreateScheduleTool(server) {
    server.tool("create_schedule", "Create one or more schedules in Revit. Supports regular schedules, material takeoffs, key schedules, view lists, sheet lists, and revision schedules. Use category ID or name to specify the element category. / 在Revit中创建一个或多个明细表。支持常规明细表、材料统计、关键字明细表、视图列表、图纸列表和修订明细表。使用类别ID或名称指定图元类别。", {
        data: z
            .array(z.object({
            name: z
                .string()
            ...
            type: z
                .string()
                .optional()
                .describe("Schedule type: regular, material, keynote, viewList, sheetList, revision / 明细表类型"),
            categoryId: z
                .number()
                .optional()
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Create schedule failed: ${error instanceof Error ? error.message : String(error)}`,
                    },
                ],
            };
        }
