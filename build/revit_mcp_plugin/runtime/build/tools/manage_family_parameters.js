...
export function registerManageFamilyParametersTool(server) {
    server.tool("manage_family_parameters", "Add, rename, remove, or set formulas for family parameters in a Revit family document. / 在Revit族文档中添加、重命名、删除或设置族参数公式。", {
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `Manage family parameters failed: ${error instanceof Error ? error.message : String(error)}`,
                    },
                ],
            };
        }
